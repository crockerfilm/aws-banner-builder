...
## Before you upload (honest TODOs)

1. **Backup image** — DV360 wants a static fallback (jpg/png) per creative. The spec-check flags this as a manual step; add `backup.jpg` to each folder. (Can't rasterize HTML offline here.)
2. **Verify current DV360 specs** — initial-load KB, polite-load, animation length/loops at support.google.com/displayvideo, then encode them in `lib/speccheck.js`.
